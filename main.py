...
#        self.bt = bowtie_table()
#        self.bt_widgets = widgets_from_df(self.bt.df)
#        self.table_widgets(self.bt_widgets,self.ids.bt_input)
#        self.dd = dd_table()
#        self.dd_widgets = widgets_from_df(self.dd.df)
#        self.table_widgets(self.dd_widgets,self.ids.dd_input)
...
#        self.update_measurement_content()
    ...
    def testy(self):
#        table_widgets(self.dd_widgets,self.ids.dd_input)
#        table_widgets(self.bt_widgets,self.ids.bt_input)
        self.bt = bowtie_table()
        self.bt_widgets = widgets_from_df(self.bt.df)
        table_widgets(self.bt_widgets,self.ids.bt_input)
        self.dd = dd_table()
    ...
    def update_data_from_widgets(self,widgets,df):
        pass
...
def table_widgets(widgets,holder):
    holder.clear_widgets()
    
    cols = list(reversed(list(widgets.keys())))
    rows = list(widgets[cols[0]].keys())
    holder.cols = len(cols)
    for i in cols:
        holder.add_widget(Label(text=i))
    for j in rows:
        for i in cols:
            holder.add_widget(widgets[i][j])
...
class bowtie_table():
    def __init__(self,df='default'):
        if df=='default':
            self.df = pd.DataFrame.from_dict({'kvs':[80,100,120,140],'body':[],'head':[]},orient='index')
    ...
    def add_row(self):
        s = self.index_data.shape
        self.df.loc[s[0]+1] = ['' for i in self.df.columns]
        
    def add_column(self):
        s = self.df.shape
        self.df[''] = ['' for i in range(s[0])]
        
    def remove_column(self):
        _gone = self.df.pop(self.df.columns[-1])
            
    def remove_row(self):
        self.df = self.df[0:-1]
            
    def save_table(self,fn):
        self.df.to_excel(fn)
        
        

        
def widgets_from_df(df):
    widgets = {}
    for c in df:
        widgets[c] = {}
        for j in range(df.shape[1]):
            print(df[c][j])
            widgets[c][j] = TextInput(text = str(df[c][j]))
    return widgets


        
def df_from_widgets(widgets):
    df = {}
    for c in widgets:
        df[c]={}
        for j in widgets[c]:
            df[c] = widgets[c][j].text
    return pd.DataFrame.from_dict(df)
        

test = {'body': {0: 'a', 1: 'b', 2: 'c', 3: 'd'},
 'head': {0: '1', 1: '2', 2: '3', 3: '4'},
 'kvs': {0: 80.0, 1: 100.0, 2: 120.0, 3: 140.0}}

t = pd.DataFrame.from_dict(test)

t2 = widgets_from_df(t)

        
class dd_table():
    def __init__(self,df='default'):
        if df=='default':
            self.df = pd.DataFrame.from_dict({'kvs':[80,100,120,140],'depth dose file':['','','','']})
        else:
            self.df = df
    def add_row(self):
        s = self.index_data.shape
...
#self.df = pd.DataFrame.from_dict({'lc':[''],'ls':[''],'AP':[''],'LR':[''],'Angle':[''],'D/CTDIvol':['']})
